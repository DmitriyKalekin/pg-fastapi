...
@router.post("/", response_model=Issue, responses={422: {"model": Error}, 404: {"model": Error}})
...
async def put_issue(uc: AIssuesUC, task_id: str = Path(...), req: CreateIssue = Body(...)):
    try:
...
async def patch_issue(uc: AIssuesUC, task_id: str = Path(...), req: CreateIssue = Body(...)):
    try:
...
@router.delete("/{task_id}", response_model=DeleteIssue, responses={404: {"model": Error}})
